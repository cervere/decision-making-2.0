...
def eachset(cueset, sb):
    ...
    for st in cueset:
        ...
        ax.plot(sals_for_plot, perfs.mean(axis=1), c=clr, lw=1, label='Stimulus '+ possible_cues[stim] + '('+stim+')')
        ...
        i += 1
    ax.plot((0,sals_for_plot[-1]), (0.5,0.5), c='black', lw=2)
    ...
    for st in cueset:
        ...
        ax.plot(delays , perfs.mean(axis=1), c=clr, lw=1, label='Stimulus '+ possible_cues[stim] + '('+stim+')')
        ...
        i += 1
    ax.plot((0,delays[-1]), (0.5,0.5), c='black', lw=2)
    ax.legend(loc=1, prop={'size':6})
