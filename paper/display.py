...
def display_ctx(stim, history, duration=3.0, sp=1, filename="decisiontime.pdf"):
    fig = plt.figure()#1, figsize=(6,5))
    plt.suptitle(stim)
    plt.subplots_adjust(top=0.15, bottom=0.1)

    timesteps = np.linspace(0,duration, len(history))

    fig.patch.set_facecolor('.9')
    ax = plt.subplot(2,1,sp)
    #if sp < 3 : ax.set_title("Cognitive Cortex")
    ax.set_title("Cognitive Cortex")
    ax.set_ylabel("Activity (Hz)")
    ax.set_xlabel("Time (seconds)")

    n = history["CTX"]["cog"][0].size
    for i in range(n):
        ax.plot(timesteps, history["CTX"]["cog"][:,i],c=colors[i], lw=1, label=shape[i])
    ax.legend(loc='upper left', fontsize=9, fancybox=True)
    #if sp > 4: ax.set_xlabel("Time (seconds)")
#    plt.plot(timesteps, history["CTX"]["cog"][:,i+1],c=colors[i+1], lw=3, label="Cognitive Cortex")

    ax = plt.subplot(2,1,sp+1)
    #if sp < 3 : ax.set_title("Motor Cortex")
    ax.set_title("Motor Cortex")
    ax.set_ylabel("Activity (Hz)")
    ax.set_xlabel("Time (seconds)")
    n = history["CTX"]["mot"][0].size
    for i in range(n):
        ax.plot(timesteps, history["CTX"]["mot"][:,i],c=colors[i], label=pos[i])
    ax.legend(loc='upper left', fontsize=9, fancybox=True)
    #if sp > 4: ax.set_xlabel("Time (seconds)")
