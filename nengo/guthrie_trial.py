...
cogstim = np.array([cues_cog[:2]])
motstim = np.array([cues_mot[:2]])
...
def learn(ctxcogact, ctxmotact, strcogact, debug=True):
    global P, R, W
    # A motor decision has been made
    c1, c2 = cues_cog[:2]
    m1, m2 = cues_mot[:2]
    mot_choice = np.argmax(ctxmotact)
    cog_choice = np.argmax(ctxcogact)
    ...
    dw = error * lrate * strcogact[choice]
...
for i in range(10):    
    ...
    c1, c2 = cues_cog[:2]
    m1, m2 = cues_mot[:2]
    ctxcogact, ctxmotact, strcogact, wt = runTrial()
    learn(ctxcogact[0], ctxmotact[0], strcogact[0], debug=False)
    print "cues (%d, %d) shown at (%d, %d)" % (c1, c2, m1, m2)
    print "Cognitive Cortex activity - %s" % str(ctxcogact[0])
    print "Motor Cortex activity - %s" % str(ctxmotact[0])
    print "Cognitive Striatum activity - %s" % str(strcogact[0])
